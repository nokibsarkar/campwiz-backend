...
func NewRoundRoutes(parent *gin.RouterGroup) {
	...
	r.POST("/import/:roundId/csv", WithSession(ImportFromCSV))
	r.POST("/import/:roundId/fountain", WithSession(ImportFromFountain))
	r.POST("/import/:roundId/campwizv1", WithSession(ImportFromCampWizV1))
...
func NewCampaignRoutes(parent *gin.RouterGroup) {
	...
	r.GET("/statistics", FetchCampaignStatistics)
...
func NewCategoryRoutes(parent *gin.RouterGroup) *gin.RouterGroup {
	...
	r.GET("/uncategorized/:campaignId", WithSession(GetUncategorizedSubmissions))
